...
const Products = () => {
  ...
  const filteredAndSortedProducts = useMemo(() => {
    ...
    // TO'G'RILANGAN KATEGORIYA FILTERI - yangi versiya
    if (categoryFilter) {
      const categoryId = parseInt(categoryFilter);
      filtered = filtered.filter((product) => {
        // Sizning ma'lumotlaringizda category ID raqam sifatida
    ...
    filtered.sort((a, b) => {
      ...
      switch (sortBy) {
        ...
        case "stock":
          ...
          break;
        case "category":
          aValue = (a.category_name || a.category?.name || "").toLowerCase();
          bValue = (b.category_name || b.category?.name || "").toLowerCase();
          break;
        case "created":
  ...
  const totalPages = Math.max(
    1,
    Math.ceil(filteredAndSortedProducts.length / pageSize)
  );
  ...
  const createProductMutation = useMutation(productService.createProduct, {
    onSuccess: () => {
      ...
      toast.success(
        t("products.product_created", "Product created successfully!")
      );
      setShowModal(false);
      resetForm();
    },
    onError: (error) => {
      toast.error(
        error.response?.data?.error ||
          t("products.create_failed", "Failed to create product")
      );
    },
  });
  ...
  const updateProductMutation = useMutation(
    ({ id, data }) => productService.updateProduct(id, data),
    {
      onSuccess: () => {
        ...
        queryClient.invalidateQueries("productStats");
        toast.success(
          t("products.product_updated", "Product updated successfully!")
        );
        setShowModal(false);
      ...
      onError: (error) => {
        toast.error(
          error.response?.data?.error ||
            t("products.update_failed", "Failed to update product")
        );
      },
    }
  );
  ...
  const deleteProductMutation = useMutation(productService.deleteProduct, {
    onSuccess: () => {
      ...
      queryClient.invalidateQueries("productStats");
      toast.success(
        t("products.product_deleted", "Product deleted successfully!")
      );
    },
    onError: (error) => {
      toast.error(
        error.response?.data?.error ||
          t("products.delete_failed", "Failed to delete product")
      );
    },
  });
  ...
  const createCategoryMutation = useMutation(productService.createCategory, {
    onSuccess: () => {
      ...
      toast.success(
        t("products.category_created", "Category created successfully!")
      );
    ...
    onError: (error) => {
      toast.error(
        error.response?.data?.error ||
          t("products.category_failed", "Failed to create category")
      );
    },
  });
  ...
  const handleExport = (format) => {
    ...
    toast.success(
      t(
        "products.export_success",
        `Products exported as ${format.toUpperCase()} successfully!`
      )
    );
  };
            ...
            <div className="space-y-3">
              ...
              <div className="flex justify-between items-center text-sm">
                ...
                <Badge variant="outline" className="text-xs">
                  {product.min_installments}-{product.max_installments}
                  {t("products.months")}
                </Badge>
              </div>
    ...
    <div className="space-y-6">
      ...
      <Card className={`${showMobileFilters ? "block" : "hidden lg:block"}`}>
        <CardHeader>
          ...
          <CardDescription>{t("products.filters_description")}</CardDescription>
        </CardHeader>
                        ...
                        <TableRow key={product.id}>
                          ...
                          <TableCell>
                            <Badge variant="outline">
                              ...
                              {product.max_installments} {t("products.months")}
                            </Badge>
                          </TableCell>
                          <TableCell>
                            <Badge
                  ...
                  <div className="space-y-2 flex items-end">
                    <label className="flex items-center space-x-2">
                      ...
                      <span className="text-sm font-medium">
                        {t("products.active_product", "Active Product")}
                      </span>
                    </label>
                  </div>
                </div>
          ...
          >
            ...
            <div className="flex items-center justify-end p-6 border-t space-x-3">
              ...
              <Button
                ...
                disabled={
                  createCategoryMutation.isLoading || !categoryForm.name.trim()
                }
              >
